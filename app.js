...
class MotionSecurityCamera {
    ...
    setupBlackScreenListeners() {
        ...
        if (!overlay) {
            console.log('Black screen overlay not found');
            return;
        }
        
        // Use a simpler double-tap detection
        overlay.addEventListener('click', (e) => {
            ...
            if (timeDiff < 500 && timeDiff > 0) {
                // Double tap detected - deactivate black screen
        ...
        // Add keyboard shortcut (ESC key) to exit black screen
        document.addEventListener('keydown', (e) => {
            if (this.blackScreenActive && e.key === 'Escape') {
                console.log('ESC key pressed - deactivating black screen');
                this.deactivateBlackScreen();
            }
        });
    }
    ...
    activateBlackScreen() {
        ...
        // Show initial hint for 2 seconds
        textElement.style.display = 'block';
        setTimeout(() => {
            if (this.blackScreenActive && textElement) {
                textElement.style.display = 'none';
            }
        }, 2000);
    }
    ...
    onMotionDetected(motionLevel) {
        ...
        // Show motion indicator (only if black screen is not active)
    ...
    showToast(message, type = 'info') {
        // Don't show toast messages when black screen is active
        if (this.blackScreenActive && type !== 'info') {
            return;
        }
        ...
        toast.style.cssText = `
            ...
            padding: 1rem 1.5rem;
            border-radius: 8px;
            z-index: ${this.blackScreenActive ? 10001 : 10000};
            animation: slideUp 0.3s ease-out;
            border-left: 4px solid ${
                type === 'success' ? '#27ae60' :
                type === 'error' ? '#e74c3c' :
                type === 'warning' ? '#f39c12' : '#4a90e2'
            };
        `;
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden && app && app.motionDetectionActive) {
        console.log('Page hidden, motion detection continues');
    }
});
